    ...
    if solution.success:
        ...
        try:
            ...
            if np.min(x1_p1) < -0.8 - 1e-6:
                print("⚠️  State bound x₁ ≥ -0.8 violated")
            else:
            ...
            print("\nControl analysis:")
            print(
                f"Phase 1 control range: [{np.min(u1_data):.3f}, {np.max(u1_data):.3f}] (bounds: [-1, 1])"
            )
            print(
                f"Phase 2 control range: [{np.min(u2_data):.3f}, {np.max(u2_data):.3f}] (unconstrained)"
            )
