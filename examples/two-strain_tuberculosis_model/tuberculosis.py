...
phase.dynamics({
    ...
    T: (u1 * r1 * L1 - mu * T +
        (1 - (1 - u2) * (p + q)) * r2 * I1 -
        beta2 * T * (I1 / N) - beta_star * T * (I2 / N)),

    L1: (beta1 * S * (I1 / N) - (mu + k1) * L1 - u1 * r1 * L1 +
         (1 - u2) * p * r2 * I1 + beta2 * T * (I1 / N) -
         beta_star * L1 * (I2 / N)),

    L2: ((1 - u2) * q * r2 * I1 - (mu + k2) * L2 +
         beta_star * (S + L1 + T) * (I2 / N)),
...
if solution.success:
    ...
    print(f"Reference: 5152.07310 (Error: {abs(solution.objective - 5152.07310) / 5152.07310 * 100:.3f}%)")
