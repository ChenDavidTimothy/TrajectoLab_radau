...
def apply_scaled_collocation_constraints(
    opti: CasadiOpti,
    mesh_interval_index: int,
    state_at_nodes: CasadiMatrix,
    control_variables: CasadiMX,
    ...
    dynamics_function: DynamicsCallable,
    problem_parameters: ProblemParameters,
    scaling_manager,  # Type hint left off for compatibility if scaling.py isn't available
) -> None:
    """
    Apply collocation constraints with scaling per Rules 1-3 from scale.txt.

    This implements equation (4.248): W_f = V_y, setting defect scaling equal to state scaling.
    ...
    Args:
        ...
        dynamics_function: Dynamics function
        problem_parameters: Problem parameters
        scaling_manager: Scaling manager for applying constraint scaling
    """
    num_states = state_at_nodes.shape[0]
    num_colloc_nodes = len(basis_components.collocation_nodes)
    colloc_nodes_tau = basis_components.collocation_nodes.flatten()
    ...
    for i_colloc in range(num_colloc_nodes):
        ...
        # Apply scaled collocation constraints - critical part implementing Rule 3
        for i_state in range(num_states):
            ...
            # Apply defect scaling - Rule 3: W_f = V_y
            scaled_defect = scaling_manager.defect_scales[i_state] * defect_constraint
