...
def _process_trajectory_points(
    mesh_interval_index, casadi_solution_object, opti_object, 
    variables_list, local_tau_values, global_normalized_mesh_nodes,
    ...
    for node_index in range(num_nodes):
        ...
        global_tau = (segment_end - segment_start) / 2 * local_tau + (segment_end + segment_start) / 2
        physical_time = (terminal_time - initial_time) / 2 * global_tau + (terminal_time + initial_time) / 2
        
        # Determine if we should add this point
        is_last_point = (mesh_interval_index == len(variables_list) - 1 and node_index == num_nodes - 1)
...
def _extract_and_format_solution(
    ...
    for mesh_interval_index in range(num_mesh_intervals):
        if not hasattr(casadi_optimization_problem_object, 'state_at_local_approximation_nodes_all_intervals_variables'):
            print("Error: state_at_local_approximation_nodes_all_intervals_variables not found in optimization object")
            continue
        ...
        last_time_point_added_to_state_trajectory = _process_trajectory_points(
            ...
            casadi_optimization_problem_object.metadata_local_state_approximation_nodes_tau[mesh_interval_index],
    ...
    for mesh_interval_index in range(num_mesh_intervals):
        if not hasattr(casadi_optimization_problem_object, 'control_at_local_collocation_nodes_all_intervals_variables'):
            print("Error: control_at_local_collocation_nodes_all_intervals_variables not found in optimization object")
            continue
        ...
        last_time_point_added_to_control_trajectory = _process_trajectory_points(
            ...
            casadi_optimization_problem_object.metadata_local_collocation_nodes_tau[mesh_interval_index],
            global_normalized_mesh_nodes,
            solution.initial_time_variable,
            solution.terminal_time_variable,
...
def solve_single_phase_radau_collocation(problem_definition: OptimalControlProblem) -> OptimalControlSolution:
    ...
    state_at_global_mesh_nodes_variables = [opti.variable(num_states) for _ in range(num_mesh_intervals + 1)]
    ...
    accumulated_integral_expressions = [ca.MX(0) for _ in range(num_integrals)] if num_integrals > 0 else []
    ...
    for mesh_interval_index in range(num_mesh_intervals):
        ...
        current_interval_state_columns[0] = state_at_global_mesh_nodes_variables[mesh_interval_index]
        ...
        state_at_interior_local_approximation_nodes_all_intervals_variables.append(interior_nodes_var)
        current_interval_state_columns[num_colloc_nodes] = state_at_global_mesh_nodes_variables[mesh_interval_index + 1]
        ...
        global_segment_length = global_normalized_mesh_nodes[mesh_interval_index + 1] - global_normalized_mesh_nodes[mesh_interval_index]
        if global_segment_length <= 1e-9:
            raise ValueError(f"Mesh interval {mesh_interval_index} has zero or negative length in global tau: {global_segment_length}")
            
        tau_to_time_scaling = (terminal_time_variable - initial_time_variable) * global_segment_length / 4.0
        ...
        for i_colloc in range(num_colloc_nodes):
            ...
            control_at_colloc = control_at_local_collocation_nodes_all_intervals_variables[mesh_interval_index][:, i_colloc]

            # Map from local tau to global tau to physical time
            local_tau = colloc_nodes_tau[i_colloc]
            global_tau = global_segment_length / 2 * local_tau + (global_normalized_mesh_nodes[mesh_interval_index + 1] + global_normalized_mesh_nodes[mesh_interval_index]) / 2
            physical_time = (terminal_time_variable - initial_time_variable) / 2 * global_tau + (terminal_time_variable + initial_time_variable) / 2

            # Apply dynamics constraint
            state_derivative_rhs = dynamics_function(state_at_colloc, control_at_colloc, physical_time, problem_parameters)
            state_derivative_rhs_vector = _validate_dynamics_output(state_derivative_rhs, num_states)
            
            opti.subject_to(state_derivative_at_colloc[:, i_colloc] == tau_to_time_scaling * state_derivative_rhs_vector)

            # Apply path constraints if provided
            if path_constraints_function:
                path_constraints = path_constraints_function(state_at_colloc, control_at_colloc, physical_time, problem_parameters)
            ...
            for integral_index in range(num_integrals):
                ...
                for i_colloc in range(num_colloc_nodes):
                    ...
                    integrand_value = integral_integrand_function(
                        state_at_colloc, control_at_colloc, physical_time, integral_index, problem_parameters
                    )
    ...
    if num_integrals > 0 and integral_integrand_function:
        if num_integrals == 1:
            ...
            for i in range(num_integrals):
                opti.subject_to(integral_decision_variables[i] == accumulated_integral_expressions[i])
    ...
    objective_value = objective_function(
        initial_time_variable, terminal_time_variable, 
        initial_state, terminal_state, 
        integral_decision_variables, problem_parameters
    )
    ...
    if problem_definition.initial_guess:
        ...
        if ig.states and len(ig.states) > 0:
            # First point
            if len(ig.states) > 0 and isinstance(ig.states[0], np.ndarray) and ig.states[0].shape[0] == num_states:
            ...
            for k in range(num_mesh_intervals):
                if k < len(ig.states) and isinstance(ig.states[k], np.ndarray):
                    ...
                    if ig.states[k].shape[0] == num_states and cols >= 0:
                        opti.set_initial(state_at_global_mesh_nodes_variables[k + 1], ig.states[k][:, cols])
            
            # Interior points
            for k in range(num_mesh_intervals):
                interior_nodes = num_collocation_nodes_per_interval[k] - 1
                if interior_nodes > 0 and k < len(ig.states) and isinstance(ig.states[k], np.ndarray) and ig.states[k].shape[0] == num_states:
                    ...
                    if guess_cols > 1:  # At least some interior points available
                        ...
                        if cols_to_use > 0 and state_at_interior_local_approximation_nodes_all_intervals_variables[k] is not None:
                            guess_slice = ig.states[k][:, 1:1+cols_to_use]
                            
                            # Pad if needed
                            if cols_to_use < interior_nodes:
                                padding = np.tile(guess_slice[:, -1:], (1, interior_nodes - cols_to_use))
                                guess_slice = np.hstack((guess_slice, padding))
                                
                            opti.set_initial(state_at_interior_local_approximation_nodes_all_intervals_variables[k], guess_slice)
                    ...
                    if control_guess.shape[0] == num_controls:
                        ...
                        if cols_to_use > 0:
                            ...
                            if cols_to_use < nodes_needed:
                                padding = np.tile(guess_slice[:, -1:], (1, nodes_needed - cols_to_use))
                                guess_slice = np.hstack((guess_slice, padding))
                                
                            opti.set_initial(control_at_local_collocation_nodes_all_intervals_variables[k], guess_slice)

        # Integrals
        if ig.integrals is not None and integral_decision_variables is not None:
            _set_initial_value_for_integrals(opti, integral_decision_variables, ig.integrals, num_integrals)
    ...
    try:
        ...
        solution = _extract_and_format_solution(
            solver_solution, opti, problem_definition, 
            num_collocation_nodes_per_interval, global_normalized_mesh_nodes
        )
