...
class Solution:
    ...
    def _extract_solution_data(self, solution):
        ...
        if self.initial_time is not None and self.final_time is not None and self._mesh_points_normalized is not None:
    ...
    def get_data_for_interval(self, interval_index) -> Optional[IntervalData]:
        ...
        nk_interval = self._polynomial_degrees[interval_index] if self._polynomial_degrees and interval_index < len(self._polynomial_degrees) else -1
    ...
    def _get_state_index(self, state_name_or_index):
        if isinstance(state_name_or_index, int):
            return state_name_or_index if 0 <= state_name_or_index < len(self._state_names) else None
    ...
    def _get_control_index(self, control_name_or_index):
        if isinstance(control_name_or_index, int):
            return control_name_or_index if 0 <= control_name_or_index < len(self._control_names) else None
