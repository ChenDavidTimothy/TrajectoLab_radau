...
from trajectolab.radau import compute_radau_collocation_components, compute_barycentric_weights, evaluate_lagrange_polynomial_at_point
...
class PolynomialInterpolant:
    ...
    def __init__(self, 
        ...
        self.bary_weights = compute_barycentric_weights(self.nodes_array) if barycentric_weights is None else np.asarray(barycentric_weights)
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                solution.solved_state_trajectories_per_interval = [
                    _extract_and_prepare_array(raw_sol.value(var), current_problem.num_states, current_nodes_list[i] + 1)
                ...
                if current_problem.num_controls > 0:
                    solution.solved_control_trajectories_per_interval = [
                        _extract_and_prepare_array(raw_sol.value(var), current_problem.num_controls, current_nodes_list[i])
                ...
                try:
                    ...
                    if current_problem.num_controls > 0:
                        ...
                        if Nk not in control_weights_cache:
                            control_weights_cache[Nk] = compute_barycentric_weights(basis.collocation_nodes)
                    ...
                    if control_evaluators[k] is None:
                        control_evaluators[k] = get_polynomial_interpolant(
                            np.array([-1.0, 1.0]),
                            np.full((current_problem.num_controls if current_problem.num_controls > 0 else 0, 2), np.nan),
                            None
                        )
            ...
            for k in range(num_intervals):
                ...
                error = calculate_relative_error_estimate(
                    k, sim_bundle, 
                    gamma_factors if current_problem.num_states > 0 else np.array([])
                )
                    ...
                    if k < num_intervals - 1:
                        ...
                        if not (np.isnan(next_error) or np.isinf(next_error)) and next_error <= error_tol:
                            ...
                            if (state_evaluators[k] is not None and 
                                ...
                                (current_problem.num_controls == 0 or (control_evaluators[k] is not None and 
                                                                 control_evaluators[k+1] is not None))):
                                
                                # Attempt h-reduction (interval merging)
                                can_merge = h_reduce_intervals(
                                    k, solution, current_problem, self.adaptive_params,
                                    gamma_factors if current_problem.num_states > 0 else np.array([]),
