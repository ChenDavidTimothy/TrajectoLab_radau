...
def _calculate_merge_feasibility_from_errors(
    ...
    # VECTORIZED: NaN checks and max calculation
    ...
    return (max_error <= error_tol and max_error != np.inf), float(max_error)
...
def p_refine_interval(max_error: float, current_Nk: int, error_tol: float, N_max: int) -> PRefineResult:
    """Determine new polynomial degree using p-refinement."""
    if max_error <= error_tol:
        return PRefineResult(current_Nk, False, current_Nk)

    nodes_to_add = max(1, N_max - current_Nk) if np.isinf(max_error) else max(1, int(np.ceil(np.log10(max_error / error_tol))))
...
def p_reduce_interval(current_Nk: int, max_error: float, error_tol: float, N_min: int, N_max: int) -> PReduceResult:
...
def h_reduce_intervals(
    phase_id: PhaseID, first_idx: int, solution: OptimalControlSolution, problem: ProblemProtocol,
    adaptive_params: AdaptiveParameters, gamma_factors: FloatArray,
    ...
    if (solution.raw_solution is None or phase_id not in solution.phase_mesh_nodes or
        first_idx + 2 >= len(solution.phase_mesh_nodes[phase_id])):
        return False
    ...
    if (phase_id not in solution.phase_initial_times or phase_id not in solution.phase_terminal_times):
        return False
    ...
    def _get_control_value(control_evaluator: Callable[[float | FloatArray], FloatArray] | None, local_tau: float) -> FloatArray:
    ...
    def merged_fwd_rhs(local_tau_k: float, state: FloatArray) -> FloatArray:
        ...
        global_tau = map_local_interval_tau_to_global_normalized_tau(local_tau_k, tau_start_k, tau_shared)
        dynamics_result = phase_dynamics_function(ca.MX(np.clip(state, -1e6, 1e6)), ca.MX(u_val), ca.MX(alpha * global_tau + alpha_0))
        return cast(FloatArray, scaling_k * _convert_casadi_dynamics_result_to_numpy(dynamics_result, num_states))
    ...
    def merged_bwd_rhs(local_tau_kp1: float, state: FloatArray) -> FloatArray:
        ...
        global_tau = map_local_interval_tau_to_global_normalized_tau(local_tau_kp1, tau_shared, tau_end_kp1)
        dynamics_result = phase_dynamics_function(ca.MX(np.clip(state, -1e6, 1e6)), ca.MX(u_val), ca.MX(alpha * global_tau + alpha_0))
        return cast(FloatArray, scaling_kp1 * _convert_casadi_dynamics_result_to_numpy(dynamics_result, num_states))

    # Get initial and terminal states
    try:
        if (phase_id in solution.phase_solved_state_trajectories_per_interval and
            first_idx < len(solution.phase_solved_state_trajectories_per_interval[phase_id])):
            initial_state_fwd = solution.phase_solved_state_trajectories_per_interval[phase_id][first_idx][:, 0].flatten()
            terminal_state_bwd = solution.phase_solved_state_trajectories_per_interval[phase_id][first_idx + 1][:, -1].flatten()
        else:
            # Fallback extraction
            ...
            Xk_nlp = ensure_2d_array(raw_sol.value(variables.phase_variables[phase_id].state_matrices[first_idx]),
                                   num_states, phase_def.collocation_points_per_interval[first_idx] + 1)
            Xkp1_nlp = ensure_2d_array(raw_sol.value(variables.phase_variables[phase_id].state_matrices[first_idx + 1]),
                                     num_states, phase_def.collocation_points_per_interval[first_idx + 1] + 1)
            initial_state_fwd, terminal_state_bwd = Xk_nlp[:, 0].flatten(), Xkp1_nlp[:, -1].flatten()
    except Exception:
        return False

    # Simulation setup
    num_sim_points = adaptive_params.num_error_sim_points
    target_end_tau_k = map_local_tau_from_interval_k_plus_1_to_equivalent_in_interval_k(1.0, tau_start_k, tau_shared, tau_end_kp1)
    target_end_tau_kp1 = map_local_tau_from_interval_k_to_equivalent_in_interval_k_plus_1(-1.0, tau_start_k, tau_shared, tau_end_kp1)

    fwd_tau_points = np.linspace(-1.0, target_end_tau_k, max(2, num_sim_points // 2), dtype=np.float64)
    bwd_tau_points = np.linspace(1.0, target_end_tau_kp1, max(2, num_sim_points // 2), dtype=np.float64)

    configured_ode_solver = adaptive_params.get_ode_solver()

    # Forward simulation
    try:
        fwd_sim = configured_ode_solver(merged_fwd_rhs, t_span=(-1.0, target_end_tau_k), y0=initial_state_fwd, t_eval=fwd_tau_points)
    ...
    # Backward simulation
    try:
        bwd_sim = configured_ode_solver(merged_bwd_rhs, t_span=(1.0, target_end_tau_kp1), y0=terminal_state_bwd, t_eval=bwd_tau_points)
        bwd_success = bwd_sim.success
        bwd_trajectory = np.array(bwd_sim.y[:, ::-1], dtype=np.float64) if bwd_success else np.full((num_states, len(bwd_tau_points)), np.nan, dtype=np.float64)
    except (RuntimeError, OverflowError, FloatingPointError):
    ...
    all_fwd_errors = np.concatenate([
        _calculate_trajectory_errors_with_gamma(
            fwd_trajectory[:, i],
            state_evaluator_first(zeta_k) if -1.0 <= zeta_k <= 1.0 + 1e-9 else
            state_evaluator_second(map_local_tau_from_interval_k_to_equivalent_in_interval_k_plus_1(zeta_k, tau_start_k, tau_shared, tau_end_kp1)),
            gamma_factors
    ...
    all_bwd_errors = np.concatenate([
        _calculate_trajectory_errors_with_gamma(
            bwd_trajectory[:, i],
            state_evaluator_second(zeta_kp1) if -1.0 - 1e-9 <= zeta_kp1 <= 1.0 else
            state_evaluator_first(map_local_tau_from_interval_k_plus_1_to_equivalent_in_interval_k(zeta_kp1, tau_start_k, tau_shared, tau_end_kp1)),
            gamma_factors
        ) for i, zeta_kp1 in enumerate(np.flip(bwd_tau_points))
    ])

    can_merge, _ = _calculate_merge_feasibility_from_errors(all_fwd_errors, all_bwd_errors, adaptive_params.error_tolerance)
    return can_merge
