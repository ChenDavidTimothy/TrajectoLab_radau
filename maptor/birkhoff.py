...
def _verify_interpolant_equivalence(
    basis_components: BirkhoffBasisComponents,
    y_initial: float,
    y_final: float,
    y_derivatives: FloatArray,
    test_points: FloatArray | None = None,
) -> bool:
    """Verify equivalence of a-form and b-form interpolants when condition is satisfied.

    From Proposition in Section 2: The a- and b-forms are equivalent if and only if:
    y(τ^b) = y(τ^a) + Σ_{j=0}^N w^B_j ẏ(τ_j)

    This function checks if this condition holds and verifies interpolant equivalence.
    """
    if test_points is None:
        # Use grid points as default test points
        test_points = basis_components.grid_points

    # Check equivalence condition: y(τ^b) = y(τ^a) + Σ_{j=0}^N w^B_j ẏ(τ_j)
    expected_y_final = y_initial + np.dot(
        basis_components.birkhoff_quadrature_weights, y_derivatives
    )
    condition_satisfied = abs(y_final - expected_y_final) < 1e-10

    if condition_satisfied:
        # Verify that interpolants are actually equivalent at test points
        a_form_values = _evaluate_birkhoff_interpolation_a_form(
            basis_components, y_initial, y_derivatives, test_points
        )
        b_form_values = _evaluate_birkhoff_interpolation_b_form(
            basis_components, y_final, y_derivatives, test_points
        )

        max_difference = np.max(np.abs(a_form_values - b_form_values))
        return max_difference < 1e-10

    return False  # Equivalence condition not satisfied
