...
class TestAdaptiveSystemValidation:
    ...
    def test_adaptive_robustness_on_challenging_problems(self):
        """
        CRITICAL: Test adaptive algorithm robustness on challenging optimal control problems.

        This test validates that the adaptive algorithm can handle a variety of
        challenging problem types without failure.
        """
        challenging_problems = []

        # Problem 1: Highly nonlinear dynamics
        problem1 = tl.Problem("Nonlinear Dynamics")
        _t1 = problem1.time(initial=0.0, final=2.0)
        x1 = problem1.state("x", initial=1.0, final=0.1)
        u1 = problem1.control("u", boundary=(-5.0, 5.0))

        # Nonlinear dynamics
        problem1.dynamics({x1: -(x1**3) + u1})
        integrand1 = x1**2 + 0.1 * u1**2
        integral1 = problem1.add_integral(integrand1)
        problem1.minimize(integral1)

        challenging_problems.append(("Nonlinear Dynamics", problem1))

        # Problem 2: Bang-bang-like control (nearly minimum time)
        problem2 = tl.Problem("Bang-Bang Control")
        _t2 = problem2.time(initial=0.0, final=1.0)
        x2 = problem2.state("x", initial=0.0, final=1.0)
        v2 = problem2.state("v", initial=0.0, final=0.0)
        u2 = problem2.control("u", boundary=(-1.0, 1.0))

        # Double integrator: nearly minimum time
        problem2.dynamics({x2: v2, v2: u2})
        integrand2 = 1.0 + 0.01 * u2**2  # Nearly minimum-time
        integral2 = problem2.add_integral(integrand2)
        problem2.minimize(integral2)

        challenging_problems.append(("Bang-Bang Control", problem2))

        # Problem 3: State-constrained problem
        problem3 = tl.Problem("State Constrained")
        _t3 = problem3.time(initial=0.0, final=1.0)
        x3 = problem3.state("x", initial=0.0, final=1.0, boundary=(0.0, 0.8))  # State constraint
        u3 = problem3.control("u")

        problem3.dynamics({x3: u3})
        integrand3 = u3**2
        integral3 = problem3.add_integral(integrand3)
        problem3.minimize(integral3)

        challenging_problems.append(("State Constrained", problem3))

        # Test each challenging problem
        successful_solves = 0
        total_problems = len(challenging_problems)

        for problem_name, problem in challenging_problems:
            try:
                # Set initial mesh
                problem.set_mesh([4, 4, 4], np.array([-1.0, -0.2, 0.3, 1.0]))
                ...
                if solution.success:
                    ...
                    # Basic validation for successful solutions
                    assert solution.objective is not None, (
                        f"{problem_name}: Missing objective value"
                    )
                    assert np.isfinite(solution.objective), f"{problem_name}: Objective not finite"
                else:
                    warnings.warn(f"{problem_name} failed: {solution.message}", stacklevel=2)

            except Exception as e:
                warnings.warn(f"{problem_name} raised exception: {e!s}", stacklevel=2)

        # CRITICAL VALIDATION: Adaptive algorithm should handle most challenging problems
        success_rate = successful_solves / total_problems
        assert success_rate >= 0.5, (
            f"Adaptive algorithm not robust enough: {successful_solves}/{total_problems} "
            f"problems solved successfully ({success_rate:.1%})"
        )

        # CRITICAL VALIDATION: At least one problem should be solved
        assert successful_solves > 0, "Adaptive algorithm failed on all challenging problems"
