...
class AdaptiveParameters:
    ...
    epsilon_tol: float  # Error tolerance threshold
    M_max_iterations: int  # Maximum number of refinement iterations
    N_min_poly_degree: int  # Minimum polynomial degree allowed
    N_max_poly_degree: int  # Maximum polynomial degree allowed
    ode_solver_tol: float = 1e-7  # ODE solver tolerance
    ...
    def __post_init__(self):
        ...
        for field_name, field_val in self.__dict__.items():
            ...
            if field_val.ndim == 1:
                ...
                raise ValueError(f"Field {field_name} must be 1D or 2D array, got {field_val.ndim}D.")


def _extract_and_prepare_array(
        casadi_value: Any,  # CasADi DM or numeric
        expected_rows: int,  # num_states or num_controls
        expected_cols: int   # Nk+1 for states, Nk for controls
    ) -> np.ndarray:
    """
    ...
    if hasattr(casadi_value, 'to_DM'):
        np_array = np.array(casadi_value.to_DM())
    else:
        np_array = np.array(casadi_value)
...
class PolynomialInterpolant:
    ...
    def __init__(self, 
        ...
        Args:
            nodes: Points at which function values are known
            values: Function values at nodes. Shape (num_vars, num_nodes)
            barycentric_weights: Pre-computed barycentric weights for nodes
        """
        # Convert to arrays if needed and ensure 2D values
        self.values_at_nodes = np.atleast_2d(values)
        self.nodes_array = np.asarray(nodes)
        ...
        if self.num_nodes_pts != self.num_nodes_val:
            raise ValueError(f"Mismatch in number of nodes ({self.num_nodes_pts}) and values columns ({self.num_nodes_val})")
        
        # Compute or use provided barycentric weights
        self.bary_weights = _compute_barycentric_weights(self.nodes_array) if barycentric_weights is None else np.asarray(barycentric_weights)
    ...
    def __call__(self, points: Union[float, np.ndarray]) -> np.ndarray:
        ...
        Args:
            points: Point(s) at which to evaluate the interpolant
        ...
        result = np.zeros((self.num_vars, len(zeta_arr)))
...
def _simulate_dynamics_for_error_estimation(
    ...
    """
    result = IntervalSimulationBundle(are_forward_and_backward_simulations_successful=False)  # Default to failure

    if not solution.success or solution.raw_solution is None:
        print(f"    Warning: NLP solution unsuccessful for interval {interval_idx} in error simulation.")
        return result
    ...
    def dynamics_rhs(tau: float, state: np.ndarray) -> np.ndarray:
        ...
        state_deriv_np = np.array(state_deriv, dtype=float).flatten()
        
        if state_deriv_np.shape[0] != num_states:
            raise ValueError(f"Dynamics function output dimension mismatch. Expected {num_states}, got {state_deriv_np.shape[0]}.")
    ...
    fwd_tau_points = np.linspace(-1, 1, n_eval_points)
    fwd_sim = ode_solver(
        dynamics_rhs, t_span=(-1, 1), y0=initial_state, 
        t_eval=fwd_tau_points, method='RK45', 
        rtol=ode_rtol, atol=ode_rtol*1e-2
    )
    
    result.forward_simulation_local_tau_evaluation_points = fwd_tau_points
    result.state_trajectory_from_forward_simulation = fwd_sim.y if fwd_sim.success else np.full((num_states, len(fwd_tau_points)), np.nan)
    if not fwd_sim.success:
        print(f"    Fwd IVP fail int {interval_idx}: {fwd_sim.message}")
    
    result.nlp_state_trajectory_evaluated_at_forward_simulation_points = state_evaluator(fwd_tau_points)
    ...
    bwd_tau_points = np.linspace(1, -1, n_eval_points)
    bwd_sim = ode_solver(
        dynamics_rhs, t_span=(1, -1), y0=terminal_state, 
        t_eval=bwd_tau_points, method='RK45', 
        rtol=ode_rtol, atol=ode_rtol*1e-2
    )
    
    result.backward_simulation_local_tau_evaluation_points = np.flip(bwd_tau_points)
    result.state_trajectory_from_backward_simulation = np.fliplr(bwd_sim.y) if bwd_sim.success else np.full((num_states, len(bwd_tau_points)), np.nan)
    if not bwd_sim.success:
        print(f"    Bwd TVP fail int {interval_idx}: {bwd_sim.message}")
    
    result.nlp_state_trajectory_evaluated_at_backward_simulation_points = state_evaluator(result.backward_simulation_local_tau_evaluation_points)
...
def calculate_relative_error_estimate(
    ...
    if not sim_bundle.are_forward_and_backward_simulations_successful or \
       sim_bundle.state_trajectory_from_forward_simulation is None or \
       sim_bundle.nlp_state_trajectory_evaluated_at_forward_simulation_points is None or \
       sim_bundle.state_trajectory_from_backward_simulation is None or \
       sim_bundle.nlp_state_trajectory_evaluated_at_backward_simulation_points is None:
        print(f"    Interval {interval_idx}: Simulation results incomplete or failed. Returning np.inf.")
        return np.inf
...
def _map_global_normalized_tau_to_local_interval_tau(global_tau: float, global_start: float, global_end: float) -> float:
...
def _map_local_interval_tau_to_global_normalized_tau(local_tau: float, global_start: float, global_end: float) -> float:
...
def _map_local_tau_from_interval_k_to_equivalent_in_interval_k_plus_1(
    local_tau_k: float, 
    global_start_k: float, 
    global_shared: float, 
    global_end_kp1: float
) -> float:
...
def h_reduce_intervals(
    ...
    control_evaluator_first: Optional[Callable],
    state_evaluator_second: Callable, 
    control_evaluator_second: Optional[Callable]
) -> bool:
    """
    ...
    error_tol = adaptive_params.epsilon_tol
    ode_rtol = adaptive_params.ode_solver_tol
    ...
    def merged_fwd_rhs(local_tau_k: float, state: np.ndarray) -> np.ndarray:
        ...
        global_tau = _map_local_interval_tau_to_global_normalized_tau(local_tau_k, tau_start_k, tau_shared)
        t_actual = alpha * global_tau + alpha_0
        
        f_rhs = dynamics_function(state_clipped, u_val, t_actual, problem_parameters)
        f_rhs_np = np.array(f_rhs, dtype=float).flatten()
    ...
    def merged_bwd_rhs(local_tau_kp1: float, state: np.ndarray) -> np.ndarray:
        ...
        state_clipped = np.clip(state, -1e6, 1e6)
        global_tau = _map_local_interval_tau_to_global_normalized_tau(local_tau_kp1, tau_shared, tau_end_kp1)
        t_actual = alpha * global_tau + alpha_0
    ...
    try:
        ...
        if Y_solved_list and first_idx < len(Y_solved_list):
            ...
            Xk_nlp_raw = raw_sol.value(opti.state_at_local_approximation_nodes_all_intervals_variables[first_idx])
    ...
    num_fwd_pts = max(
        num_sim_points // 2, 
        int(num_sim_points * (target_end_tau_k - (-1.0)) / 2.0) if target_end_tau_k > -1.0 else num_sim_points // 2
    )
    num_fwd_pts = max(2, num_fwd_pts)
    fwd_tau_points = np.linspace(-1.0, target_end_tau_k, num_fwd_pts)

    print(f"      h-reduction: Starting Merged IVP sim from zeta_k=-1 to {target_end_tau_k:.3f} ({num_fwd_pts} pts)")
    try:
        fwd_sim = solve_ivp(
            merged_fwd_rhs, t_span=(-1.0, target_end_tau_k), 
            y0=initial_state_fwd, t_eval=fwd_tau_points, 
            method='RK45', rtol=ode_rtol, atol=ode_atol
        )
        fwd_trajectory = fwd_sim.y if fwd_sim.success else np.full((num_states, len(fwd_tau_points)), np.nan)
        if not fwd_sim.success:
            print(f"      Merged IVP failed: {fwd_sim.message}")
    except Exception as e:
        print(f"      Exception during merged IVP simulation: {e}")
        fwd_trajectory = np.full((num_states, len(fwd_tau_points)), np.nan)

    # Get terminal state for backward simulation
    try:
        if Y_solved_list and (first_idx + 1) < len(Y_solved_list):
            Xkp1_nlp = Y_solved_list[first_idx + 1]
        else:  # Fallback
            opti = solution.opti_object
            raw_sol = solution.raw_solution
            Xkp1_nlp_raw = raw_sol.value(opti.state_at_local_approximation_nodes_all_intervals_variables[first_idx+1])
    ...
    num_bwd_pts = max(
        num_sim_points // 2, 
        int(num_sim_points * (1.0 - target_end_tau_kp1) / 2.0) if target_end_tau_kp1 < 1.0 else num_sim_points // 2
    )
    num_bwd_pts = max(2, num_bwd_pts)
    bwd_tau_points = np.linspace(1.0, target_end_tau_kp1, num_bwd_pts)

    print(f"      h-reduction: Starting Merged TVP sim from zeta_kp1=1 to {target_end_tau_kp1:.3f} ({num_bwd_pts} pts)")
    try:
        bwd_sim = solve_ivp(
            merged_bwd_rhs, t_span=(1.0, target_end_tau_kp1), 
            y0=terminal_state_bwd, t_eval=bwd_tau_points, 
            method='RK45', rtol=ode_rtol, atol=ode_atol
        )
        sorted_bwd_tau_points = np.flip(bwd_tau_points)
        bwd_trajectory = np.fliplr(bwd_sim.y) if bwd_sim.success else np.full((num_states, len(sorted_bwd_tau_points)), np.nan)
        if not bwd_sim.success:
    ...
    if num_states == 0:
        can_merge = ('fwd_sim' in locals() and fwd_sim.success and 
                    'bwd_sim' in locals() and bwd_sim.success)
    ...
    if np.isnan(max_error):
        print(f"      h-reduction check: max_error calculation resulted in NaN. Merge not approved.")
        max_error = np.inf 
    ...
    if can_merge:
        print(f"      h-reduction condition met. Merge approved for intervals {first_idx}, {first_idx+1}.")
    else:
        print(f"      h-reduction condition NOT met (error {max_error:.2e} > tol {error_tol:.2e}). Merge failed.")
...
def p_reduce_interval(
    ...
    if max_error > error_tol or current_Nk <= N_min:
        return PReduceResult(
            new_num_collocation_nodes=current_Nk,
    ...
    return PReduceResult(
        new_num_collocation_nodes=new_Nk,
        was_reduction_applied=was_reduced
    )
...
def _generate_robust_default_initial_guess(
    ...
    propagated_phase_initial_time_guess: Optional[float] = None,  # Keeping original param name
    propagated_phase_terminal_time_guess: Optional[float] = None, # Keeping original param name
    propagated_integral_values_guess: Optional[Union[float, np.ndarray]] = None # Keeping original param name
) -> InitialGuess:
    ...
    for idx, Nk in enumerate(collocation_nodes_list):
        ...
        if num_controls > 0:
            control_traj = np.full((num_controls, Nk), default_control)
        else:
            control_traj = np.empty((0, Nk))
        controls.append(control_traj)

    # Time variable guesses
    if propagated_phase_initial_time_guess is None and problem.initial_guess:
        propagated_phase_initial_time_guess = problem.initial_guess.initial_time_variable
    
    if propagated_phase_terminal_time_guess is None and problem.initial_guess:
        propagated_phase_terminal_time_guess = problem.initial_guess.terminal_time_variable

    # Integral guesses
    final_integral_guess = None
    if num_integrals > 0:
        if propagated_integral_values_guess is not None:
            final_integral_guess = propagated_integral_values_guess
        else:
            ...
            if problem.initial_guess and problem.initial_guess.integrals is not None:
                ...
                raw_guess = [default_integral] * num_integrals if num_integrals > 1 else default_integral

            if num_integrals == 1:
                final_integral_guess = float(raw_guess) if not isinstance(raw_guess, (list, np.ndarray)) else float(raw_guess[0])
            elif isinstance(raw_guess, (list, np.ndarray)) and len(raw_guess) == num_integrals:
                final_integral_guess = np.array(raw_guess)
            else: 
                final_integral_guess = np.full(num_integrals, default_integral)
    
    return InitialGuess(
        initial_time_variable=propagated_phase_initial_time_guess,
        terminal_time_variable=propagated_phase_terminal_time_guess,
        states=states,
        controls=controls,
...
def _propagate_guess_from_previous(
    ...
    # Using the ORIGINAL parameter names:
    guess = _generate_robust_default_initial_guess(
        problem, target_nodes_list,
        propagated_phase_initial_time_guess=t0_prop, 
        propagated_phase_terminal_time_guess=tf_prop, 
        propagated_integral_values_guess=integrals_prop
    )
...
def _calculate_gamma_normalizers(
    ...
    gamma_factors = 1.0 / np.maximum(gamma_denominator, 1e-12)  # Avoid division by zero
...
def run_phs_adaptive_mesh_refinement(
    ...
    error_tol = adaptive_params.epsilon_tol
    max_iterations = adaptive_params.M_max_iterations
    N_min = adaptive_params.N_min_poly_degree
    N_max = adaptive_params.N_max_poly_degree
    ode_rtol = adaptive_params.ode_solver_tol
    num_sim_points = adaptive_params.num_error_sim_points
    ...
    if problem.global_normalized_mesh_nodes is not None:
        current_mesh = np.array(problem.global_normalized_mesh_nodes)
    else:
    ...
    current_problem = OptimalControlProblem(
        num_states=problem.num_states,
        num_controls=problem.num_controls,
        num_integrals=problem.num_integrals,
        dynamics_function=problem.dynamics_function,
        objective_function=problem.objective_function,
        t0_bounds=problem.t0_bounds,
        tf_bounds=problem.tf_bounds,
        integral_integrand_function=problem.integral_integrand_function,
        path_constraints_function=problem.path_constraints_function,
        event_constraints_function=problem.event_constraints_function,
        problem_parameters=problem.problem_parameters,
        default_initial_guess_values=problem.default_initial_guess_values,
        solver_options=problem.solver_options
    )
    
    num_states = problem.num_states
    num_controls = problem.num_controls
    most_recent_solution = None
    ...
    for iteration in range(max_iterations):
        ...
        if not solution.success:
            error_msg = f"NLP solver failed in adaptive iteration {iteration}. " + (solution.message or "Solver error.")
        ...
        try:
            ...
            solution.solved_state_trajectories_per_interval = [
                _extract_and_prepare_array(raw_sol.value(var), num_states, current_nodes_list[i] + 1)
            ...
            if num_controls > 0:
                solution.solved_control_trajectories_per_interval = [
                    _extract_and_prepare_array(raw_sol.value(var), num_controls, current_nodes_list[i])
        ...
        state_evaluators = [None] * num_intervals
        control_evaluators = [None] * num_intervals
            ...
            try:
                ...
                if num_controls > 0:
                    ...
                    if Nk not in control_weights_cache:
                        control_weights_cache[Nk] = _compute_barycentric_weights(basis.collocation_nodes)
        ...
        for k in range(num_intervals):
            ...
            if state_eval is None or control_eval is None:
                print(f"    Warning: Missing interpolants for interval {k}. Assigning high error.")
                errors[k] = np.inf
                continue
                
            # Simulate dynamics for error estimation
            sim_bundle = _simulate_dynamics_for_error_estimation(
                k, solution, current_problem,
                state_eval, control_eval,
                ode_rtol=ode_rtol, 
            ...
            error = calculate_relative_error_estimate(
                k, sim_bundle, 
                gamma_factors if num_states > 0 else np.array([])
            )
        ...
        while k < num_intervals:
            ...
            if np.isnan(error_k) or np.isinf(error_k) or error_k > error_tol:
                ...
                if p_result.was_p_successful:
                    ...
                    print(f"        p-refinement applied: Nk {Nk} -> {p_result.actual_Nk_to_use}")
                    next_nodes_list.append(p_result.actual_Nk_to_use)
                    next_mesh.append(current_mesh[k + 1])
                    k += 1
                else:
                    # p-refinement failed, apply h-refinement
                    print(f"        p-refinement failed. Attempting h-refinement.")
                    h_result = h_refine_params(p_result.unconstrained_target_Nk, N_min)
                    
                    print(f"          h-refinement: Splitting interval {k} into {h_result.num_new_subintervals} subintervals, each Nk={h_result.collocation_nodes_for_new_subintervals[0]}.")
                    ...
                    new_nodes = np.linspace(tau_start, tau_end, h_result.num_new_subintervals + 1)
                ...
                if k < num_intervals - 1:
                    ...
                    if not (np.isnan(next_error) or np.isinf(next_error)) and next_error <= error_tol:
                        print(f"      Interval {k+1} also has low error ({next_error:.2e}). Eligible for h-reduction.")
                        
                        # Check if interpolants are available
                        if (state_evaluators[k] is not None and 
                            state_evaluators[k+1] is not None and
                            (num_controls == 0 or (control_evaluators[k] is not None and 
                                                 control_evaluators[k+1] is not None))):
                            
                            # Attempt h-reduction (interval merging)
                            can_merge = h_reduce_intervals(
                                k, solution, current_problem, adaptive_params,
                                gamma_factors if num_states > 0 else np.array([]),
                                state_evaluators[k], control_evaluators[k],
                                state_evaluators[k+1], control_evaluators[k+1]
                            )
                        else:
                            print("      Skipping h-reduction attempt due to missing interpolants.")
                ...
                if can_merge:
                    ...
                    print(f"      h-reduction failed or not applicable. Attempting p-reduction for interval {k}.")
                    p_reduce = p_reduce_interval(Nk, error_k, error_tol, N_min, N_max)
                    
                    if p_reduce.was_reduction_applied:
                        print(f"        p-reduction applied: Nk {Nk} -> {p_reduce.new_num_collocation_nodes}")
                    else:
        ...
        early_return_solution = most_recent_solution if most_recent_solution else OptimalControlSolution()
        ...
        if len(current_nodes_list) > 0:
            ...
            unique_nodes, counts = np.unique(np.round(current_mesh, decimals=12), return_counts=True)
            if np.any(counts > 1):
            ...
            if len(unique_nodes) > 1 and not np.all(np.diff(unique_nodes) > 1e-9):
                ...
                problem_pairs = ", ".join([
                    f"({unique_nodes[i]:.3f}, {unique_nodes[i+1]:.3f})" for i in problem_indices
                ]) if problem_indices.size > 0 else "N/A"
